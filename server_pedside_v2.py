...
future_pred = 4.8 # how many seconds of future coords to generate
...
def receive_ped_coords(client, userdata, msg):
    ...
    x, y = hf.get_xy_from_latlong(float(lat), float(long), place)
    ...
    if (len(ped_coords[ped_id]['x']) >= coords_to_start_traj_pred): # if the ped has sent a certain number of coords to register a traj
        ...
        for vehicle_id in vehicle_data['o']:
            veh_xnump = vehicle_data['o'][vehicle_id]['x']
            veh_ynump = vehicle_data['o'][vehicle_id]['y']
        ...
        end1 = time.time()
        # print(f">> Timeline1: {round(end1-start1, 3)}")
